...
public interface ProductRepository extends JpaRepository<Product, String>, JpaSpecificationExecutor<Product> {
    ...
    @Query(value = "select p from Product p where p.isActive = 1 order by p.createdAt desc", countQuery = "select count(p) from Product p where p.isActive = 1")
    ...
    @Query(value = "select p.name from Product p where p.skuCode = :skuCode")
    String getProductNameBySkuCode(@Param("skuCode") String skuCode);
}
